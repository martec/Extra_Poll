...
define("IN_MYBB", 1);
define('THIS_SCRIPT', 'expolls.php');

$templatelist = "changeuserbox,loginbox,expolls_newpoll_option,expolls_newpoll,expolls_editpoll_option,expolls_editpoll,expolls_showresults_resultbit,expolls_showresults";
...
$plugins->run_hooks("expolls_start");
...
if($mybb->input['action'] == "newpoll")
{
	...
	$plugins->run_hooks("expolls_newpoll_start");
	...
	for($i = 1; $i <= $polloptions; ++$i)
	{
		...
		eval("\$optionbits .= \"".$templates->get("expolls_newpoll_option")."\";");
		$option = "";
	}
	...
	$plugins->run_hooks("expolls_newpoll_end");

	eval("\$newpoll = \"".$templates->get("expolls_newpoll")."\";");
	output_page($newpoll);
}
if($mybb->input['action'] == "do_newpoll" && $mybb->request_method == "post")
{
	...
	$plugins->run_hooks("expolls_do_newpoll_start");
	...
	$plugins->run_hooks("expolls_do_newpoll_process");
...
if($mybb->input['action'] == "editpoll")
{
	...
	$plugins->run_hooks("expolls_editpoll_start");
	...
	if(empty($mybb->input['updateoptions']))
	{
		...
		for($i = 0; $i < $numoptions; ++$i)
		{
			...
			eval("\$optionbits .= \"".$templates->get("expolls_editpoll_option")."\";");
			$option = "";
			$optionvotes = "";
		}
	...
	$plugins->run_hooks("expolls_editpoll_end");

	eval("\$editpoll = \"".$templates->get("expolls_editpoll")."\";");
...
if($mybb->input['action'] == "showresults")
{
	...
	$plugins->run_hooks("expolls_showresults_start");
	...
	$query = $db->query("
		SELECT v.*, u.username
		FROM ".TABLE_PREFIX."pollvotes2 v
	...
	for($i = 1; $i <= $poll['numoptions']; ++$i)
	{
		...
		eval("\$polloptions .= \"".$templates->get("expolls_showresults_resultbit")."\";");
	}
	...
	$plugins->run_hooks("expolls_showresults_end");

	$poll['question'] = htmlspecialchars_uni($poll['question']);
	eval("\$showresults = \"".$templates->get("expolls_showresults")."\";");
...
if($mybb->input['action'] == "vote" && $mybb->request_method == "post")
{
	...
	$query = $db->simple_select("polls2", "*", "pid='".$mybb->get_input('pid')."'");
	$poll = $db->fetch_array($query);
	...
	$plugins->run_hooks("expolls_vote_start");
	...
	$query = $db->simple_select("pollvotes2", "*", "{$user_check} AND pid='".$poll['pid']."'");
	...
	if (!$mybb->binary_fields['pollvotes2']) {
		$mybb->binary_fields['pollvotes2'] = array("ipaddress" => true);
	}
	...
	if($poll['multiple'] == 1)
	{
		$db->insert_query_multiple("pollvotes2", $votesql);
	}
	else
	{
		$db->insert_query("pollvotes2", $votesql);
	}
	...
	$plugins->run_hooks("expolls_vote_process");

	$db->update_query("polls2", $updatedpoll, "pid='".$poll['pid']."'");

	$plugins->run_hooks("expolls_vote_end");
...
if($mybb->input['action'] == "do_undovote")
{
	...
	$plugins->run_hooks("expolls_do_undovote_start");
	...
	$query = $db->simple_select("pollvotes2", "vid,voteoption", "{$user_check} AND pid='".$poll['pid']."'");
	...
	$plugins->run_hooks("expolls_do_undovote_process");

	$db->delete_query("pollvotes2", "{$user_check} AND pid='".$poll['pid']."'");
	$db->update_query("polls2", $updatedpoll, "pid='".$poll['pid']."'");

	$plugins->run_hooks("expolls_do_undovote_end");
